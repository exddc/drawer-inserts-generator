...
export function useParameterChange(
    ...
    useEffect(() => {
        ...
        depth,
        maxBoxWidth,
        maxBoxDepth, // Dependencies that trigger grid regeneration
        sceneRef,
        gridHelperRef,
        setGrid, // Other necessary dependencies
    ])
    ...
    useEffect(() => {
        ...
        if (window.raycastManager) {
            ...
            try {
                ...
                if (
                    ...
                    console.log(
                        'Raycast Manager config changed (camera or target group) after box regeneration.'
                    )
                    needsInit = true
                }
            } catch (e) {
                ...
                console.warn(
                    "Could not check raycast manager's current config after box regeneration, assuming re-init needed.",
                    e
                )
                needsInit = true
            }
            ...
            if (needsInit) {
                try {
                    ...
                    console.log(
                        'Raycast Manager re-initialized after box regeneration.'
                    )
                } catch (e) {
                    console.error(
                        'Error re-initializing Raycast Manager after box regeneration:',
                        e
                    )
                }
            }
        ...
        cornerRadius,
        hasBottom,
        getBoxHexColor, // Box properties needed for generation
        width,
        depth, // Needed for positioning calculations
    ])
}
