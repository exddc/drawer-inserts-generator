...
function getBoxKey(box: THREE.Object3D): string {
    if (box.userData && box.userData.dimensions) {
        ...
        const [smallerDimension, largerDimension] = [width, depth].sort((a, b) => a - b);
...
export async function exportMultipleSTLs(
    ...
    try {
        ...
        const uniqueBoxes = new Map<string, { box: THREE.Object3D, count: number, index: number }>();
        ...
        if (inputs.uniqueBoxesExport) {
            ...
            console.log(`Found ${uniqueBoxes.size} unique box dimensions out of ${boxMeshGroup.children.length} total boxes`);
        }
        ...
        if (inputs.uniqueBoxesExport && uniqueBoxes.size > 0) {
            ...
            for (const [boxKey, boxInfo] of uniqueBoxes.entries()) {
                try {
                    ...
                    if (boxInfo.box.userData && boxInfo.box.userData.dimensions) {
                        const { width, depth, height } = boxInfo.box.userData.dimensions;
                    ...
                    console.error(`Error exporting unique box ${uniqueIndex}:`, error);
                }
            }
        } else {
