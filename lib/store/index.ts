...
export const useBoxStore = create<StoreState>()((set, get) => ({
  ...
  updateInput: (name: string, value: number | boolean | string) => {
    ...
    // Box-related inputs
    if ([
      'width', 'depth', 'height', 'wallThickness', 'cornerRadius', 'hasBottom',
      'minBoxWidth', 'maxBoxWidth', 'minBoxDepth', 'maxBoxDepth', 
      'useMultipleBoxes', 'uniqueBoxesExport'
    ].includes(name)) {
      state.updateBoxInput(name, value);
    } 
    // UI-related inputs
    else if ([
      'debugMode', 'showGrid', 'showAxes', 'boxColor', 'highlightColor'
    ].includes(name)) {
