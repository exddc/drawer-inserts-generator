...
interface BoxModelParams {
    ...
    combinedBoxes?: Map<number, CombinedBoxInfo> // Updated type
  }
...
export function createBoxModel(
    ...
    try {
      ...
      for (const [primaryIndex, combinedInfo] of combinedBoxes.entries()) {
        ...
        if (!combinedInfo || !combinedInfo.indices || !combinedInfo.direction) {
        ...
        if (direction === 'width') {
          ...
          combinedWidth = allBoxPositions.reduce((sum, pos) => sum + pos.box.width, 0);
          ...
          combinedDepth = allBoxPositions.reduce((sum, pos) => sum + pos.box.depth, 0);
